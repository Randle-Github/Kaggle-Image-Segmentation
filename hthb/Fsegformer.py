...
class Fsegformer(nn.Module):
    ...
    def forward(self, x: Tensor,ogs) -> Tensor:
        ...
        last= self.head(fpn_out)
        logit = self.logit(last)
        logit = F.interpolate(logit, size=None, scale_factor=4, mode='bilinear', align_corners=False)
        out=logit
        aux_out =[self.aux[i](fpn_out[i]) for i in range(4)]
    ...
    def __init__(
        ...
        self.mlp = nn.ModuleList([
            nn.Sequential(
                ...
                # GCT(decoder_dim,after_relu=True),
...
def Fsegformer_layers(m):
    ...
    return [
        ...
        # aspp bottleneck
        list(m.FPN.parameters()),list(m.head.parameters())+list(m.logit.parameters())+list(m.aux.parameters())
    ]


def BatchFormer(x, y, encoder, is_training):
    # x: input features with the shape [N, C,h,w]
    # x.flatten -> [N,C*H*W] -> [1,N,C*H*W]->[N, C,h,w]
...
# config.num_outs = 4
# config.mit = 2
# model=segformer_fpn(config,pretrain=False)
