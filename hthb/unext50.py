...
class FPN(nn.Module):
    ...
    def forward(self, xs:list, last_layer):
        ...
        return torch.cat(hcs, dim=1),hcs

class UnetBlock(Module):
    def __init__(self, up_in_c:int, x_in_c:int, nf:int=None, blur:bool=False,
                 self_attention:bool=False,up_out_c = None ,**kwargs):
        super().__init__()
        if up_out_c is  None:
            up_out_c = up_in_c//2
        self.shuf = PixelShuffle_ICNR(up_in_c,up_out_c , blur=blur, **kwargs)
...
class UneXt50(nn.Module):
    def __init__(self, stride=1,config=None, **kwargs):
        super().__init__()
        #encoder
        
        m = torch.load("/home/wangjingqi/hthb/models/resnext50_32x4d.pth")
    ...
    def forward(self, x,ogs):
        ...
        enc2 = self.enc2(enc1)
        enc3 = self.enc3(enc2)
        enc4 = self.enc4(enc3)#+self.organ_map0(ogs)
        enc5 = self.aspp(enc4)#+self.organ_map1(ogs)
