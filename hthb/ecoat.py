...
class ecoat(nn.Module):
    ...
    def __init__(self,
        ...
        self.embedding = Embedding3D(5,(320,256,256))

    def forward(self, x,ogs):

        encoder = self.encoder(x)#[,,,]

        last, decoder = self.head(encoder)#(N,320,H,W) + (N,320,H,W)
        last = last + self.embedding(ogs)
        logit = self.logit(last)#(N,1,H,W)
        
        logit = F.interpolate(logit, size=None, scale_factor=4, mode='bilinear', align_corners=False)
        aux_out = [self.aux[i](encoder[i]) for i in range(4)]
        output=logit
        return (output,aux_out,ogs)
...
class Embedding3D(nn.Module):
    ...
    def forward(self,inputs):
        return torch.stack([self.embedding[o-1] for o in inputs],dim=0)
...
def ecoat_layers(m):
    ...
    return [
        ...
        # aspp bottleneck
        list(m.head.parameters())+ list(m.embedding.parameters())+list(m.logit.parameters())+list(m.aux.parameters())
    ]
