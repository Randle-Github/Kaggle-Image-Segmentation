...
mit = {0: mit_b0, 2: mit_b2,3:mit_b3, 5: mit_b5}
import os
# from config import config
from CrossAttention import TransformerLearner
from fpn import FPN,ACFPN
from fpnt import FPNT
#####################MiT#######################################
class senformer(nn.Module):


    def __init__(self, config=None,pretrain=True):
        ...
        self.FPN = ACFPN(encoder_dim,fpn_dim,num_outs=config.num_outs,add_extra_convs=config.add_extra_convs)
        fpn_dim = [fpn_dim]*config.num_outs
        self.learners = nn.ModuleList()
        for dim in fpn_dim:
            self.learners.append(
                TransformerLearner(dim=dim,
                                 num_heads=8,
                                 num_queries=config.num_classes,
        ...
        self.head = SegformerDecoder(
            encoder_dim=[config.num_classes]*len(fpn_dim),
            decoder_dim=config.num_classes,
        )
        self.logit = nn.ModuleList([nn.Conv2d(config.num_classes, config.num_classes, kernel_size=1, padding=0) for _ in range(len(fpn_dim))])
        self.aux = nn.ModuleList([
            nn.Conv2d(fpn_dim[i], config.num_classes, kernel_size=1, padding=0) for i in range(len(fpn_dim))
        ])
    def forward(self, x, ogs):
        ...
        # torch.Size([2, 512, 24, 24])
        
        FPNT_out = self.FPN(x)
        aux_out = [self.aux[i](FPNT_out[i]) for i in range(4)]
        # torch.Size([2, 512, 192, 192])
        # torch.Size([2, 512, 96, 96])
        # torch.Size([2, 512, 48, 48])
        # torch.Size([2, 512, 24, 24])

        transformer_out = []
        for l,o in enumerate(FPNT_out):
            transformer_out.append(self.learners[l](o))

        head_out = self.head(transformer_out)
        out = []
        for i in range(4):
            out.append(F.interpolate(self.logit[i](head_out[i]), size=None, scale_factor=4, mode='bilinear',
                                     align_corners=False))
        end = 0.25 * (out[0] + out[1] + out[2] + out[3])
        out.append(end)
...
def senformer_layers(m):
    ...
    return [
        ...
        # aspp bottleneck
        list(m.head.parameters())+list(m.FPN.parameters())+list(m.learners.parameters()) + list(m.logit.parameters())
    ]
# from config import config
# model=MIT_Newloss(config,pretrain=False)
