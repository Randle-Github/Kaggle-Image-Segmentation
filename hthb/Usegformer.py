...
class DecoderBlock(nn.Module):
    def __init__(
        ...
        self.attention1 = Attention2(attention_type, in_channels=in_channels + skip_channels)
        ...
        self.attention2 = Attention2(attention_type, in_channels=out_channels)
...
mit = {0: mit_b0, 2: mit_b2, 5: mit_b5}
#################################################################
...
class Usegformer(nn.Module):
    ...
    def __init__(self,config = None):
        ...
        checkpoint_model = torch.load(os.path.join(config.pretrain, f"mit_b{config.mit}.pth"))
        ...
        self.logit = nn.Sequential(
            nn.Conv2d(decoder_dim[-1], config.num_classes, kernel_size=1),
        )
    ...
    def forward(self, x,ogs):
        ...
        d = self.head.center(head)

        decoder = []
        for i, decoder_block in enumerate(self.head.blocks):
...
def Usegformer_layers(m):
    ...
    return [
        ...
        # aspp bottleneck
        list(m.head.parameters())+list(m.conv.parameters())+list(m.aux.parameters())
    ]
# model=MiT_decoder_Unet(i=2)
