        ...
        if self.parallel_depth > 0:
            self.parallel_blocks = nn.ModuleList([
                ParallelBlock(
                    ...
                    shared_crpes=nn.ModuleList( [self.crpe1, self.crpe2, self.crpe3, self.crpe4, self.cpe5])
                )
...
class fcoat(nn.Module):
    ...
    def __init__(self,
                 ...
                 config,pretrain = True,fold = -1

                 ):
        super(fcoat, self).__init__()
        decoder_cfg={}
        ...
        if pretrain and fold < 0:
            checkpoint_model = torch.load(os.path.join(config.pretrain,"coat_small_7479cf9b.pth"),
            map_location="cpu")
            self.encoder.load_state_dict(checkpoint_model["model"], strict=False)

            print("Loaded checkpoint successfully!")
        elif pretrain and fold >= 0:
            checkpoint_model = torch.load(os.path.join(config.pretrain,f"backbone-{fold}.pth"),
            map_location="cpu")
        ...
        self.head = daformer_conv1x1(
            encoder_dim=fpn_dim,
            decoder_dim=decoder_dim,
        )
        self.logit = nn.Sequential(
            nn.Conv2d(decoder_dim, config.num_classes, kernel_size=1),
        )

        self.aux = nn.ModuleList([
            nn.Conv2d(encoder_dim[i], config.num_classes, kernel_size=1, padding=0) for i in range(len(encoder_dim))
        ])

    def forward(self, x,ogs):

        encoder = self.encoder(x)
        fpn_out = self.FPN(encoder)
        last,_= self.head(fpn_out)
        logit = self.logit(last)
        logit = F.interpolate(logit, size=None, scale_factor=4, mode='bilinear', align_corners=False)
        out=logit
        aux_out =[self.aux[i](encoder[i]) for i in range(len(encoder))]
        return (out,aux_out,ogs)

class Embedding3D(nn.Module):
    def __init__(self,corpus_size,output_shape):
        super().__init__()
        self.embedding =nn.parameter.Parameter( torch.randn(corpus_size,*output_shape))#(5,320,H,W)
    def forward(self,inputs):
        return torch.stack([self.embedding[o] for o in inputs],dim=0)
# model = coat(None,False)
...
def fcoat_layers(m):
    ...
    return [
        ...
        # aspp bottleneck
        list(m.head.parameters())+ list(m.FPN.parameters())+list(m.logit.parameters())+list(m.aux.parameters())
    ]
